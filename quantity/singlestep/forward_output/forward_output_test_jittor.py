...
def prepare_config():
    ...
    config_mmonitor = {
        nn.BatchNorm2d: ['ForwardOutputMean','ForwardOutputNorm','ForwardOutputStd'],
        nn.Linear: ['ForwardOutputMean','ForwardOutputNorm','ForwardOutputStd'],
        nn.Conv2d: ['ForwardOutputMean','ForwardOutputNorm','ForwardOutputStd']
    }
...
def test_linear_mean():
    ...
    print(if_similar(quantity.get_output()[0],jt.mean(y),model,name))
def test_conv_mean():
...
def test_linear_norm():
    ...
    print(if_similar(quantity.get_output()[0],jt.norm(jt.flatten(y)),model,name))
def test_conv_norm():
    cov = nn.Conv2d(1, 2, 3, 1, 1)
    x_conv = jt.randn((4, 1, 3, 3), requires_grad=True)
...
def test_linear_std():
    ...
    print(if_similar(quantity.get_output()[0],jt.std(y),model,name))
def test_conv_std():
    cov = nn.Conv2d(1, 2, 3, 1, 1)
    x_conv = jt.randn((4, 1, 3, 3), requires_grad=True)
    optimizer = optim.SGD(cov.parameters(), lr=0.01)
