...
def prepare_config():
    ...
    config_mmonitor = {
        ...
        nn.Conv2d: ['BackwardOutputNorm','BackwardOutputMean','BackwardOutputStd'],
        nn.Linear: ['BackwardOutputNorm','BackwardOutputMean','BackwardOutputStd']
    }
...
def compute_linear_norm():
    l = nn.Linear(2,3) 
    x = torch.randn(4,2,requires_grad=True)
    quantity_l = BackwardOutputNorm(l)
    for hook in quantity_l.backward_extensions():
        l.register_full_backward_hook(hook)
    i = 0
    y = l(x)
...
def compute_default_norm():
    ...
    x = torch.tensor([[-1.0, 2.0], [3.0, -4.0]], requires_grad=True)
    # 前向传播
    quantity = BackwardOutputNorm(relu)
    for hook in quantity.backward_extensions():
        relu.register_full_backward_hook(hook)
    y = relu(x)
    y.retain_grad()
